import torch, numpy as np, sys, uuid, os, gc, time
from transformers import AutoTokenizer, AutoModelForCausalLM, BitsAndBytesConfig
from gradio_client import Client

# Public Proxy URL
A_U = "https://yubainu-open.hf.space/"
M_I = "google/gemma-2b" 
    ...
    for i in range(30):
        try:
            with torch.no_grad():
                ...
                api_lat = 0.0
                try:
                    # Target explicit API name "/analyze"
                    inner_payload = {"packet": p_k, "rst": i==0, "token": TKN, "is_b": True}
                    result = api_client.predict(
                        payload=inner_payload, 
                        api_name="/analyze"
                    )
                    
                    cur_s = result.get("score", 0.0)
                    api_lat = result.get("latency_ms", 0.0)
                except Exception as e:
                    # print(f"API Error: {e}")
                    pass
                ...
                t_s = tk.decode(n_t[0])
                print(f"Token {i:02d} [{t_s.strip()}]: Score = {cur_s:.4f} | Engine: {api_lat:.2f}ms")
